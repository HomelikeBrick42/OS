            ...
            keyboard.data_state = match keyboard.data_state {
                KeyboardDataState::None => {
                    ...
                    if first == 0xE0 || first == 0xE1 {
                        KeyboardDataState::First([first])
                    } else {
                        let state = if first & 0b1000_0000 == 0 {
                            KeyState::Pressed
                        } else {
                            KeyState::Released
                        };
                        let key = match first & !0b1000_0000 {
                            0x01 => Key::Escape,
                            0x02 => Key::Num1,
                            0x03 => Key::Num2,
                            0x04 => Key::Num3,
                ...
                KeyboardDataState::First([first]) => {
                    ...
                    if first == 0xE0 {
                        ...
                        let key = match second & !0b1000_0000 {
                ...
                KeyboardDataState::Second([first, second]) => {
                    ...
                    keyboard.key_events.push_back(KeyEvent {
                        key: Key::Unknown3([first, second, third]),
                        state,
                    });
