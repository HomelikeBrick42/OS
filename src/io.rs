...
pub(crate) unsafe extern "win64" fn keyboard_interrupt() {
    unsafe {
        let scancode = input_byte(0x60);
        io_wait();

        static IS_2_BYTE_SCANCODE: AtomicBool = AtomicBool::new(false);

        if IS_2_BYTE_SCANCODE.swap(false, Ordering::Relaxed) {
            todo!("implment multibyte scancodes")
        } else if (0x01..=0xD8).contains(&scancode) {
            let key = match scancode & 0b0111_1111 {
                0x01 => Keycode::Escape,
                0x02 => Keycode::Num1,
                0x03 => Keycode::Num2,
                0x04 => Keycode::Num3,
            ...
            let action = if (scancode & 0b1000_0000) == 0 {
                Action::Pressed
            } else {
                Action::Released
            };
            ...
            IS_2_BYTE_SCANCODE.store(true, Ordering::Relaxed);
        } else if scancode == 0xE1 {
            todo!("implement pause scancode")
        }
